...
const isValidPswp = element => Array.from(element.querySelectorAll('a')).some(link => link.querySelectorAll('img').length === 1);
const bemSelectors = block => ({
...
const getUrlParam = param => {
  ...
  const result = window.location.hash.substr(1).split('&').map(p => p.split('=')).find(p => p[0] === param);
...
class Photoswiper {
  constructor(galleryElement, config) {
    ...
    if (isValidPswp(this.galleryElement)) {
      ...
      const bemSelectors$$1 = this.config.bemRoot ? bemSelectors(this.config.bemRoot) : {};
      ...
      this.items = Array.from(this.figures).map(fig => this.parseFigure(fig)); // add listeners to open on click

      this.figures.forEach(figure => {
        figure.querySelector(this.selectors.LINK).addEventListener('click', this.onClick);
      });
      this.enabled = true;
  ...
  clickHandler(e = window.event) {
    ...
    const clickFigure = clickTarget.closest(this.selectors.FIGURE) || this.galleryElement;
...
//# sourceMappingURL=photoswiper.js.map
